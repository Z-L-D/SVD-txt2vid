    ...
    def _encode_text(
        ...
        if do_classifier_free_guidance:
            if negative_prompt:
                ...
                negative_attention_mask = negative_inputs.attention_mask.to(device)
                negative_text_embeddings = self.text_encoder(negative_input_ids, negative_attention_mask)
                negative_text_embeddings = negative_text_embeddings.last_hidden_state[:, 0, :]
            else:

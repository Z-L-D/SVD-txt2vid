...
def main(args, prompts):
    ...
    # vae = AutoencoderKL.from_pretrained(args.sd_dir, subfolder="vae", low_cpu_mem_usage=False, device_map=None).to(torch_device)
    # vae.eval()
    # vis = UNet2DConditionModel.from_pretrained(args.sd_dir, subfolder="unet", low_cpu_mem_usage=False, device_map=None).to(torch_device)
    # vis.eval()
    # noise_scheduler = EulerAncestralDiscreteScheduler.from_pretrained(args.sd_dir, subfolder="scheduler", low_cpu_mem_usage=False, device_map=None)

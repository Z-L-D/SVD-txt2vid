...
# Load CLIP model from Hugging Face's transformers
...
def sample(
    ...
    text_description: str = None,  # New parameter for text description
    ...
    for input_img_path in all_img_paths:
        ...
        image_tensor = ToTensor()(input_image) * 2.0 - 1.0
        image_tensor = image_tensor.unsqueeze(0).to(device)

        if text_description:
            inputs = clip_processor(text=[text_description], return_tensors="pt", padding=True)
            text_features = clip_model.get_text_features(**inputs)  # Get text embeddings
            text_features = text_features.unsqueeze(0).to(device)  # Reshape and move to device

            # Combine image and text tensors. Adjust combination logic as needed.
            combined_tensor = torch.cat((image_tensor, text_features), dim=1)
        else:
            combined_tensor = image_tensor
        
        # H, W = image.shape[2:]
        # assert image.shape[1] == 3

        H, W = combined_tensor.shape[2:]
        ...
        value_dict["cond_frames_without_noise"] = combined_tensor
        value_dict["motion_bucket_id"] = motion_bucket_id
        value_dict["fps_id"] = fps_id
        value_dict["cond_aug"] = cond_aug
        # value_dict["cond_frames"] = image + cond_aug * torch.randn_like(image)
        value_dict["cond_frames"] = combined_tensor + cond_aug * torch.randn_like(combined_tensor)
